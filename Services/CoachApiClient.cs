...
using IdentityPendingResponse = CoachingFit.Identity.Shared.Wrappers.GenericResponse<System.Collections.Generic.IEnumerable<string>>;
...
public class CoachApiClient(HttpClient _http)
{
    ...
    {
        var resp = await _http.GetFromJsonAsync<IdentityPendingResponse>("api/Auth/coaches/pending", ct);
        return resp?.Data ?? [];
    }
